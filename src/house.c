...
Room * newRoom(char * name, double area, double areaScreen, double raioOne, double raioTwo, int wall_pos, double angulo) // Função que retorna um ponteiro com as informações alocadas para cada cômodo
{
    Room * comodo = malloc(sizeof(Room));
    strcpy(comodo->name, name);//Copia o nome
    comodo->area = area;//Copia a área de píxel
    comodo->areaScreen = areaScreen; // Copia a área a ser exibida
    comodo->delta = -1;
    ...
    comodo->next = NULL;
    return comodo; // Retorno do espaço alocado
}
